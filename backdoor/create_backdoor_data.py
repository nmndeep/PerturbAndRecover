...
def prepare_path_name(args, len_entire_dataset, start, end):
    ...
    output = start
    output += f'_{args.dataset}_multi_{args.multi}_{args.label}_{args.patch_type}_{args.patch_location}_{args.patch_size}_wd_{args.patch_width}_ns_{args.noise_coeff}'
...
def create_backdoor(args):
    config    = eval(open(args.templates).read())
    ...
    for i in tqdm(range(len(df_backdoor))):
        ...
        image = apply_trigger(image, patch_size = args.patch_size, patch_type = args.patch_type, patch_location = args.patch_location, width=args.patch_width, noise_coeff=args.noise_coeff)

        image_filename = 'SAVELOC + {folder_name}/{image_name}'          
...
if(__name__ == "__main__"):
    ...
    parser.add_argument("--patch_type", type = str, default = "random", help = "type of patch", choices = ["random", "blended", "SIG", "warped", "blended_rs", "badnet_rs", "blended_rs_multi", "blended_rs_hor", "water_patt", "tri_patt", "stock_patt"])
    parser.add_argument("--patch_location", type = str, default = "random", help = "type of patch", choices = ["random", "four_corners", "blended", "blended_rs", "blended_upd"])
    ...
    parser.add_argument("--noise_coeff", type = float, default = 0.05, help = "Patch size for backdoor images")

    parser.add_argument("--dataset", type = str, default = "laion", help = "")
